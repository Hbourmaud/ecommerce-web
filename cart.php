...
<?php
    ...
    if(isset($_GET['deleteItem'])){
        QueryToDB("DELETE FROM cart WHERE UUID =\"".$_SESSION['login']."\" AND ID_item = \"".$_GET['deleteItem']."\"");
        header('Location: ./cart');
        Exit();
    }
    if(isset($_GET['changeQuantity']) && isset($_GET['numberToChange'])){
        QueryToDB("DELETE FROM cart WHERE UUID =\"".$_SESSION['login']."\" AND ID_item = \"".$_GET['changeQuantity']."\"");
        $number = $_GET['numberToChange'];
        if($number < 1){
            $number = 1;
        }
        for ($i=0; $i < $number; $i++) {
            QueryToDB("INSERT INTO cart (UUID, ID_item) VALUES (\"".$_SESSION['login']."\" , \"".$_GET['changeQuantity']."\")");
        }
        header('Location: ./cart');
        Exit();
    }
    
    $result = QueryToDB("SELECT link_picture,name,description,price,COUNT(item.ID) AS 'nb',item.ID AS 'itemID' FROM cart INNER JOIN item ON cart.ID_item = item.ID WHERE UUID =\"".$_SESSION['login']."\" GROUP BY item.ID;");
